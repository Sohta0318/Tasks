...
const capitalize = (sentence) => {
  const newSentence = sentence[0].toUpperCase();
  return newSentence + sentence.slice(1);
};
...
const isPangram = (sentence) => {
  ...
  const long = sentence.replace(/\s/g, "");
  for (let i = 0; i <= all.length; i++) {
    if (long.indexOf(all[i]) === -1) {
      return false;
    } else {
      return true;
    }
